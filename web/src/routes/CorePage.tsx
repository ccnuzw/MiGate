...
export default function CorePage({ core }: { core: 'xray' | 'singbox' }) {
  ...
  const endpoints = core === 'xray'
    ? { status: api.xrayStatus, version: api.xrayVersion, config: api.xrayConfig, logs: api.xrayLogs, apply: api.xrayApply, install: api.xrayInstall, uninstall: api.xrayUninstall }
    : { status: api.singboxStatus, version: api.singboxVersion, config: api.singboxConfig, logs: api.singboxLogs, apply: api.singboxApply, install: api.singboxInstall, uninstall: api.singboxUninstall };
  ...
  const apply = useMutation({
    ...
    onError: (error) => showToast(errorMessage(error, `${label} 应用失败`), 'error'),
  });
  const install = useMutation({
    ...
    onError: (error) => showToast(errorMessage(error, `${label} 安装失败`), 'error'),
  });
  const uninstall = useMutation({
    ...
    onError: (error) => showToast(errorMessage(error, `${label} 卸载失败`), 'error'),
  });
...
export function coreActionResult(data: CoreActionResponse, fallback: string): { ok: boolean; message: string } {
  ...
  if (failed) return { ok: false, message: error || data.xray?.status || data.status || fallback };
  if (data.xray?.status) return { ok: true, message: `${fallback}：${data.xray.status}` };
  if (data.singbox?.applied === true || data.applied === true) return { ok: true, message: fallback };
  return { ok: true, message: data.status || fallback };
}
