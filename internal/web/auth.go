...
func authMiddleware(next http.Handler, cfg *routerConfig) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if path == "/login" || path == "/api/health" || path == "/api/login" || path == "/api/session" || path == "/api/vpngate/servers" || path == "/api/vpngate/probe" || path == "/api/vpngate/egress/capabilities" || path == "/api/vpngate/auto-health/status" || path == "/api/singbox/status" || path == "/api/singbox/version" || path == "/api/singbox/config" || strings.HasPrefix(path, "/sub/") || strings.HasPrefix(path, "/static/") {
			next.ServeHTTP(w, r)
			return
		}
