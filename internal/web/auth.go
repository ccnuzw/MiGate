...
func authMiddleware(next http.Handler, cfg *routerConfig) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if path == "/login" || path == "/api/health" || path == "/api/login" || path == "/api/session" || path == "/api/vpngate/servers" || path == "/api/vpngate/probe" || path == "/api/vpngate/auto-health/status" || strings.HasPrefix(path, "/sub/") {
			next.ServeHTTP(w, r)
			return
		}
